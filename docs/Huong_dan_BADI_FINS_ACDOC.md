...
## Kinh nghiệm thao tác BAdI

Các điểm dưới đây là checklist thao tác. Mỗi dòng đều có nguồn internet để đối chiếu; nếu nguồn chỉ là SAP Support Content / SAP Community thì không dùng thay cho kiểm tra trên hệ `DEV`.

| Kinh nghiệm | Việc cần làm trên hệ SAP | Nguồn internet |
|-------------|--------------------------|----------------|
| Ưu tiên enhancement thay vì modification | Dùng BAdI/enhancement nếu SAP có điểm mở rộng; không sửa source SAP standard nếu chưa có quyết định kỹ thuật riêng | [SAP Help — Enhancement Concept](https://help.sap.com/saphelp_ewm900/helpdata/en/42/d356adddec036fe10000000a114cbd/content.htm) |
| Luôn kiểm single-use / multiple-use trước khi active | Trong `SE18` / `SE19`, xem BAdI có cho nhiều implementation active không. Nếu single-use, chỉ để một implementation active cho cùng runtime/filter | [SAP Help — Steps to Activate BAdI Implementations](https://help.sap.com/saphelp_SCM700_ehp01/helpdata/en/4e/43e679eb811552e10000000a421937/content.htm) |
| Với single-use BAdI, kiểm fallback/default trước khi debug lỗi runtime | Nếu gặp `CX_BADI_NOT_IMPLEMENTED`, kiểm fallback class/default implementation hoặc active implementation trong `SE19` | [SAP Help — Building Your First BAdI](https://help.sap.com/docs/SAP_NETWEAVER_AS_ABAP_FOR_SOH_740/46a2cfc13d25463b8b9a3d2a3c3ba0d9/44f5175e19fd2463e10000000a1553f7.html?locale=en-US&state=PRODUCTION&version=7.40.28), [SAP Help — Enhancement Framework FAQs](https://help.sap.com/docs/ABAP_PLATFORM_2021/46a2cfc13d25463b8b9a3d2a3c3ba0d9/a31d40425c459923e10000000a155106.html) |
| Với filter-dependent BAdI, debug filter trước khi sửa code | Trong `SE18` / `SE19`, kiểm filter type và filter value. Khi debug `GET BADI`, ghi lại filter runtime để so với filter của implementation | [SAP Help — Definition of a Filter-Dependent Classic BAdI](https://help.sap.com/docs/SAP_NETWEAVER_AS_ABAP_FOR_SOH_740/2b28ffa716c24348903f8ffbfeb81df8/36a4b575ebb99a459b8ffc79a3c9ffcc.html?version=7.40.25), [SAP ABAP — GET BADI](https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABAPGET_BADI.html) |
| Không copy code mẫu khi chưa xem signature thật | Mở `SE24` → interface BAdI → method → tab **Parameters**. Chỉ gán dữ liệu vào parameter có hướng phù hợp, ví dụ `CHANGING` nếu cần sửa table/list runtime | [SAP ABAP — CALL BADI](https://help.sap.com/doc/abapdocu_cp_index_htm/CLOUD/en-US/ABAPCALL_BADI.html) |
| Tìm BAdI bằng runtime, không chỉ bằng tên package | Dùng `SE93` lấy program, search source, rồi debug `GET BADI` / `CALL BADI`; với classic BAdI kiểm `CL_EXITHANDLER` | [SAP Help Support Content — What BAdI is and how to find and implement it](https://help.sap.com/docs/SUPPORT_CONTENT/abap/3353525767.html), [SAP Help — Breakpoints at Statements](https://help.sap.com/saphelp_em900/helpdata/en/49/26e11dc93016b8e10000000a42189d/content.htm) |
| Tránh `COMMIT WORK`, RFC gây implicit commit, và update trực tiếp table chuẩn trong BAdI nếu SAP không cho phép | Không thêm `COMMIT WORK` trong implementation `ZCL_*`. Nếu cần ghi dữ liệu riêng, thiết kế theo extension point được SAP cho phép và test rollback/update termination | [SAP Help — How to deal with Customer Exits and BAdIs](https://help.sap.com/docs/SUPPORT_CONTENT/erpscm/3362167802.html), [SAP Help — Basics of BAdIs](https://help.sap.com/docs/SUPPORT_CONTENT/srm/3362937811.html) |
| Sau transport, kiểm lại trạng thái active trên target system | Trên QA/PRD, mở `SE19` kiểm implementation active, `SE24` kiểm class active, chạy lại flow thật (`FAGLGVTR` hoặc job/app BCF) | [SAP Help — Steps to Activate BAdI Implementations](https://help.sap.com/saphelp_SCM700_ehp01/helpdata/en/4e/43e679eb811552e10000000a421937/content.htm) |
| Với SAP S/4HANA Cloud, kiểm BAdI có allow-listed/released không | Không giả định BAdI on-premise dùng được trên Public Cloud. Kiểm danh sách allow-listed BAdIs hoặc extensibility document theo scope item | [SAP Help — Enhancements: Allow-Listed BAdIs](https://help.sap.com/docs/SAP_S4HANA_CLOUD/8bae74e1ba0c4c8cae3a97f15b76ad1e/c498c72fed494d46a5c4df8fa4eb8bec.html) |
...
- SAP Help — Enhancement Framework FAQs: https://help.sap.com/docs/ABAP_PLATFORM_2021/46a2cfc13d25463b8b9a3d2a3c3ba0d9/a31d40425c459923e10000000a155106.html — dùng để xác minh fallback class, default implementation, context-dependent BAdI và instance reuse.
- SAP Help — Steps to Activate BAdI Implementations: https://help.sap.com/saphelp_SCM700_ehp01/helpdata/en/4e/43e679eb811552e10000000a421937/content.htm — dùng để xác minh active/inactive implementation, transport và giới hạn single-use BAdI.
- SAP Help — Building Your First BAdI: https://help.sap.com/docs/SAP_NETWEAVER_AS_ABAP_FOR_SOH_740/46a2cfc13d25463b8b9a3d2a3c3ba0d9/44f5175e19fd2463e10000000a1553f7.html?locale=en-US&state=PRODUCTION&version=7.40.28 — dùng để xác minh fallback class cho single-use BAdI.
- SAP Help — Definition of a Filter-Dependent Classic BAdI: https://help.sap.com/docs/SAP_NETWEAVER_AS_ABAP_FOR_SOH_740/2b28ffa716c24348903f8ffbfeb81df8/36a4b575ebb99a459b8ffc79a3c9ffcc.html?version=7.40.25 — dùng để xác minh filter type và filter value trong filter-dependent BAdI.
- SAP Help — How to deal with Customer Exits and BAdIs: https://help.sap.com/docs/SUPPORT_CONTENT/erpscm/3362167802.html — dùng để xác minh rủi ro `COMMIT WORK`, RFC implicit commit, rollback không đầy đủ và update trực tiếp table chuẩn trong enhancement.
- SAP Help — Basics of BAdIs: https://help.sap.com/docs/SUPPORT_CONTENT/srm/3362937811.html — dùng để xác minh khuyến nghị không commit trong custom BAdI implementation.
- SAP Help — Enhancement Concept: https://help.sap.com/saphelp_ewm900/helpdata/en/42/d356adddec036fe10000000a114cbd/content.htm — dùng để xác minh enhancement option là hook, enhancement ít tốn công khi upgrade hơn modification, và BAdI thuộc enhancement spot.
- SAP Help — Enhancements: Allow-Listed BAdIs: https://help.sap.com/docs/SAP_S4HANA_CLOUD/8bae74e1ba0c4c8cae3a97f15b76ad1e/c498c72fed494d46a5c4df8fa4eb8bec.html — dùng để xác minh hướng kiểm allow-listed BAdIs trong SAP S/4HANA Cloud.
